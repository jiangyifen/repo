 ...
 * @since 0.3
 */
public abstract class AbstractManagerEventListener implements
	...
	protected void handleEvent(ChannelReloadEvent event) {
	}
	...
	public void onManagerEvent(ManagerEvent event) {
		...
		if (event instanceof AgentCallbackLoginEvent) {
			...
			handleEvent((QueueCallerAbandonEvent) event);
		...
		} else if (event instanceof ChannelReloadEvent) {
			handleEvent((ChannelReloadEvent) event);
		} else {
			logger.info("UNKNOW EVENT: " + event);
		}
	}
}
