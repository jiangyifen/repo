 ...
 * www.dhtmlgoodies.com. You will find this and a lot of other scripts at our
 * website. Terms of use: You are free to use this script as long as the
 * copyright message is kept intact. However, you may not redistribute, sell or
 * repost it without our permission. Thank you! www.dhtmlgoodies.com Alf Magne
 * Kalleland
 * ***********************************************************************************************************
 */
...
function moveSingleElement() {
	...
	for ( var no = 0; no < tmpFromBox.options.length; no++) {
		if (tmpFromBox.options[no].selected) {
			...
			tmpToBox.options[tmpToBox.options.length] = new Option(
					tmpFromBox.options[no].text, tmpFromBox.options[no].value);
...
function createMovableOptions(fromBox, toBox, totalWidth, totalHeight,
